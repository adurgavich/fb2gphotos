(function(){"use strict";var that={};that.version="1.0.4";that.remove=function(jpeg){var b64=!1;if(jpeg.slice(0,2)=="\xff\xd8"){}else if(jpeg.slice(0,23)=="data:image/jpeg;base64,"||jpeg.slice(0,22)=="data:image/jpg;base64,"){jpeg=atob(jpeg.split(",")[1]);b64=!0}else{throw new Error("Given data is not jpeg.")}
...
return new_data};that.insert=function(exif,jpeg){var b64=!1;if(exif.slice(0,6)!="\x45\x78\x69\x66\x00\x00"){throw new Error("Given data is not exif.")}
if(jpeg.slice(0,2)=="\xff\xd8"){}else if(jpeg.slice(0,23)=="data:image/jpeg;base64,"||jpeg.slice(0,22)=="data:image/jpg;base64,"){jpeg=atob(jpeg.split(",")[1]);b64=!0}else{throw new Error("Given data is not jpeg.")}
...
return exif_dict};that.dump=function(exif_dict_original){var TIFF_HEADER_LENGTH=8;var exif_dict=copy(exif_dict_original);var header="Exif\x00\x00\x4d\x4d\x00\x2a\x00\x00\x00\x08";var exif_is=!1;var gps_is=!1;var interop_is=!1;var first_is=!1;var zeroth_ifd,exif_ifd,interop_ifd,gps_ifd,first_ifd;if("0th" in exif_dict){zeroth_ifd=exif_dict["0th"]}else{zeroth_ifd={}}
if((("Exif" in exif_dict)&&(Object.keys(exif_dict.Exif).length))||(("Interop" in exif_dict)&&(Object.keys(exif_dict.Interop).length))){zeroth_ifd[34665]=1;exif_is=!0;exif_ifd=exif_dict.Exif;if(("Interop" in exif_dict)&&Object.keys(exif_dict.Interop).length){exif_ifd[40965]=1;interop_is=!0;interop_ifd=exif_dict.Interop}else if(Object.keys(exif_ifd).indexOf(that.ExifIFD.InteroperabilityTag.toString())>-1){delete exif_ifd[40965]}}else if(Object.keys(zeroth_ifd).indexOf(that.ImageIFD.ExifTag.toString())>-1){delete zeroth_ifd[34665]}
if(("GPS" in exif_dict)&&(Object.keys(exif_dict.GPS).length)){zeroth_ifd[that.ImageIFD.GPSTag]=1;gps_is=!0;gps_ifd=exif_dict.GPS}else if(Object.keys(zeroth_ifd).indexOf(that.ImageIFD.GPSTag.toString())>-1){delete zeroth_ifd[that.ImageIFD.GPSTag]}
if(("1st" in exif_dict)&&("thumbnail" in exif_dict)&&(exif_dict.thumbnail!=null)){first_is=!0;exif_dict["1st"][513]=1;exif_dict["1st"][514]=1;first_ifd=exif_dict["1st"]}
var zeroth_set=_dict_to_bytes(zeroth_ifd,"0th",0);var zeroth_length=(zeroth_set[0].length+exif_is*12+gps_is*12+4+zeroth_set[1].length);var exif_set,exif_bytes="",exif_length=0,gps_set,gps_bytes="",gps_length=0,interop_set,interop_bytes="",interop_length=0,first_set,first_bytes="",thumbnail;if(exif_is){exif_set=_dict_to_bytes(exif_ifd,"Exif",zeroth_length);exif_length=exif_set[0].length+interop_is*12+exif_set[1].length}
...
if(interop_is){var pointer_value=(TIFF_HEADER_LENGTH+zeroth_length+exif_length+gps_length);var pointer_str=pack(">L",[pointer_value]);var key=40965;var key_str=pack(">H",[key]);var type_str=pack(">H",[TYPES.Long]);var length_str=pack(">L",[1]);interop_pointer=key_str+type_str+length_str+pointer_str}
if(first_is){var pointer_value=(TIFF_HEADER_LENGTH+zeroth_length+exif_length+gps_length+interop_length);first_ifd_pointer=pack(">L",[pointer_value]);var thumbnail_pointer=(pointer_value+first_set[0].length+24+4+first_set[1].length);var thumbnail_p_bytes=("\x02\x01\x00\x04\x00\x00\x00\x01"+pack(">L",[thumbnail_pointer]));var thumbnail_length_bytes=("\x02\x02\x00\x04\x00\x00\x00\x01"+pack(">L",[thumbnail.length]));first_bytes=(first_set[0]+thumbnail_p_bytes+thumbnail_length_bytes+"\x00\x00\x00\x00"+first_set[1]+thumbnail)}
var zeroth_bytes=(zeroth_set[0]+exif_pointer+gps_pointer+first_ifd_pointer+zeroth_set[1]);if(exif_is){exif_bytes=exif_set[0]+interop_pointer+exif_set[1]}
return(header+zeroth_bytes+exif_bytes+gps_bytes+interop_bytes+first_bytes)};function copy(obj){return JSON.parse(JSON.stringify(obj))}
...
var littleEndian;if(mark[0]=="<"){littleEndian=!0}else if(mark[0]==">"){littleEndian=!1}else{throw new Error("")}
...
if((val>0xff)||(val<0)){throw new Error("'pack' error.")}else{valStr=String.fromCharCode(val)}}else if(c=="H"){val=array[p-1];if((val>0xffff)||(val<0)){throw new Error("'pack' error.")}else{valStr=String.fromCharCode(Math.floor((val%0x10000)/0x100))+String.fromCharCode(val%0x100);if(littleEndian){valStr=valStr.split("").reverse().join("")}}}else if(c.toLowerCase()=="l"){val=array[p-1];if((c=="l")&&(val<0)){val+=0x100000000}
if((val>0xffffffff)||(val<0)){throw new Error("'pack' error.")}else{valStr=String.fromCharCode(Math.floor(val/0x1000000))+String.fromCharCode(Math.floor((val%0x1000000)/0x10000))+String.fromCharCode(Math.floor((val%0x10000)/0x100))+String.fromCharCode(val%0x100);if(littleEndian){valStr=valStr.split("").reverse().join("")}}}else{throw new Error("'pack' error.")}
packed+=valStr;p+=1}
return packed}
...
var littleEndian;if(mark[0]=="<"){littleEndian=!0}else if(mark[0]==">"){littleEndian=!1}else{throw new Error("'unpack' error.")}
...
val=sliced.charCodeAt(0)*0x100+sliced.charCodeAt(1)}else if(c.toLowerCase()=="l"){length=4;sliced=str.slice(strPointer,strPointer+length);if(littleEndian){sliced=sliced.split("").reverse().join("")}
val=sliced.charCodeAt(0)*0x1000000+sliced.charCodeAt(1)*0x10000+sliced.charCodeAt(2)*0x100+sliced.charCodeAt(3);if((c=="l")&&(val>=0x80000000)){val-=0x100000000}}else{throw new Error("'unpack' error. "+c)}
...
var head=2;var segments=["\xff\xd8"];while(!0){if(data.slice(head,head+2)=="\xff\xda"){segments.push(data.slice(head));break}else{var length=unpack(">H",data.slice(head+2,head+4))[0];var endPoint=head+length+2;segments.push(data.slice(head,endPoint));head=endPoint}
